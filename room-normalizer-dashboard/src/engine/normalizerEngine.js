...
function pass2AlternativeSingle(tokens) {
  for (const { start, end, window } of patternWindows(tokens, 3)) {
    ...
    if (canonA === canonB) {
      if (catA === "BEDDING_TYPE") return { category: catA, text: `2 ${canonA} Beds` };
      if (catA === "VIEW_CORE") return { category: catA, text: `${canonA} View` };
      return { category: catA, text: canonA };
    }
    if (catA === "VIEW_CORE") return { category: catA, text: `${canonA} or ${canonB} View` };
    return { category: catA, text: `${canonA} or ${canonB}` };
  }
  return null;
}
...
function pass3(tokens, auditSink, rawName, diag, telemetry) {
  const buckets = { class: [], occupancy: [], type: [], building: [], amenity: [], conditional: [], custom: [] };
  ...
  for (const t of tokens) {
    ...
    if (t.resolved) {
      ...
      else if (category === "CONDITIONAL") buckets.conditional.push(canonical);
...
async function assemble(buckets, bedroom, beddingList, viewList) {
  ...
  const conditionalList = dedup(buckets.conditional);
  ...
  parts.push(...buildingList, ...amenityList, ...customList, ...conditionalList);
  ...
  return {
    ...
    parsed_components: {
      ...
      conditional: conditionalList.length ? conditionalList : null,
      custom: customList,
    },
  };
}
...
export async function normalizeCore(rawName, dict, options = {}) {
  ...
  const altAmenity = alternatives.filter((a) => a.category === "AMENITY").map((a) => a.text);
  ...
  for (const text of altAmenity) buckets.amenity.push(text);
