...
export function buildLookup(rows) {
  ...
  for (const row of rows) {
    for (const rawSyn of row.synonyms.split(",")) {
      ...
      // ENGINE EXTENSION — Access pre-pass vocab. Every word/phrase that
      // structurally belongs to something else acts as a stop-boundary for
      // the generic "<words> access" capture; every ACCESSIBILITY-bucket
      // word is an exclusion so accessibility phrases stay untouched;
      // ACCESS_MODIFIER words (e.g. "limited") layer onto whatever's
      // captured.
      if (["CLASS", "OCCUPANCY", "TYPE", "BUILDING", "BEDDING_TYPE", "NOISE"].includes(row.category)) {
