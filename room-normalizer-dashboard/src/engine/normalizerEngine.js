...
function isClassOrViewCoreWord(t) { return t.resolved?.category === "CLASS" || t.resolved?.category === "VIEW_CORE"; }
...
function pass2BuildingSingle(tokens) {
  for (const { start, end, window } of patternWindows(tokens, 2)) {
    ...
    const [a, b] = window;
    if (isBuildingAnchor(a) && isClassOrViewCoreWord(b)) { claim(tokens, start, end, "R_BUILDING"); return fmtBuilding(b, a); }
    if (isClassOrViewCoreWord(a) && isBuildingAnchor(b)) { claim(tokens, start, end, "R_BUILDING"); return fmtBuilding(a, b); }
  }
  return null;
}
