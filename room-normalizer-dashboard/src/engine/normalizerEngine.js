...
function pass1(tokens, dict, diag, telemetry) {
  for (const { start, end, window } of unclaimedWindows(tokens, dict.maxPhraseLen)) {
    ...
    if (!match) continue;
    const { termId, canonical, category, action, outputBucket, triggerBucket } = match;
    ...
    if (window.length === 1) {
      ...
      window[0].resolved = { termId, canonical, category, action, outputBucket, triggerBucket };
    } else {
      claim(tokens, start, end, `dict:${category}:REPLACE`);
      window.forEach((t) => (t.resolved = { termId, canonical, category, action, outputBucket, triggerBucket }));
    }
...
function pass2AlternativeSingle(tokens) {
  for (const { start, end, window } of patternWindows(tokens, 3)) {
    ...
    if (canonA === canonB) {
      if (catA === "BEDDING_TYPE") return { category: catA, text: `2 ${canonA} Beds`, outputBucket: a.resolved?.outputBucket };
      if (catA === "VIEW_CORE") return { category: catA, text: `${canonA} View`, outputBucket: a.resolved?.outputBucket };
      return { category: catA, text: canonA, outputBucket: a.resolved?.outputBucket };
    }
    if (catA === "VIEW_CORE") return { category: catA, text: `${canonA} or ${canonB} View`, outputBucket: a.resolved?.outputBucket };
    return { category: catA, text: `${canonA} or ${canonB}`, outputBucket: a.resolved?.outputBucket };
  }
  return null;
}
...
function pass3(tokens, auditSink, rawName, diag, telemetry) {
  ...
  // assemble() to apply.
  const buckets = { class: [], occupancy: [], type: [], building: [], accessibility: [], other: [], custom: [], triggers: [] };
  ...
  for (const t of tokens) {
    ...
    if (t.resolved) {
      flushUnresolvedRun();
      const { canonical, category, outputBucket, triggerBucket } = t.resolved;
      if (category === "CLASS") buckets.class.push(canonical);
        ...
        if (triggerBucket) {
          ...
          buckets.accessibility.push(canonical);
    ...
    diag?.emit(
      ...
      isOrphanNumber
        ...
        : "No dictionary or structural rule resolved this token, so it was emitted as custom output.",
      { bucket: isOrphanNumber ? null : "custom" }
    );
  }
  flushUnresolvedRun();
  return { buckets };
...
async function assemble(buckets, bedroom, beddingList, viewList) {
  ...
  const accessibilityList = dedup(buckets.accessibility);
  ...
  if (accessibilityList.length) segments.push(accessibilityList.join(", "));
  if (otherList.length) segments.push(otherList.join(", "));
  // ENGINE EXTENSION — `custom` (genuinely undefined leftover text) no
  ...
  return {
    ...
    parsed_components: {
      ...
      accessibility: accessibilityList.length ? accessibilityList : null,
...
export async function normalizeCore(rawName, dict, options = {}) {
  ...
  for (const alt of altAmenity) {
    if (alt.outputBucket === "ACCESSIBILITY") buckets.accessibility.push(alt.text);
...
export const DICTIONARY_CATEGORIES = [
  ...
  "BUILDING", "AMENITY", "PRIVILEGE", "CONDITIONAL", "NOISE",
];
...
// don't map 1:1 to a single output group.
export const OUTPUT_BUCKETS = ["ROOM", "BEDROOM", "BEDDING", "VIEW", "BUILDING", "ACCESSIBILITY", "OTHER", "NOT_NEEDED"];
