...
// these helpers are new parsing rules added after reviewing real
...
// silently diverge. (Also un-ported: the Building-anchor pairing pass
// (pass2BuildingSingle/All) and the View->CLASS default-fallback rule in
// pass3 — both added this cycle, see their definitions below.)
// ------------------------------------------------------------

// "king/twin" -> "king or twin" so the alternative-preservation pass (below)
// can treat slash and the word "or" identically. Tolerates whitespace
// around the slash ("king / twin" behaves identically to "king/twin") so
// alternation isn't silently missed just because a supplier added spaces.
// Explicitly excludes "w/" (NOISE synonym for "with") by exact word check
// rather than relying on tight spacing to avoid it, since that's a fixed
// idiom, not a real alternative, regardless of spacing.
function preprocessSlashAlternatives(sanitized) {
  return sanitized.replace(/\b([a-z0-9]+)\s*\/\s*([a-z0-9]+)\b/g, (match, left, right) => {
    if (left === "w") return match;
...
// ENGINE EXTENSION — some BEDDING_TYPE canonical terms already contain the
// word "Bed" themselves (e.g. "Sofa Bed"), so appending "Bed"/"Beds" for
// the count suffix would double it ("1 Sofa Bed Bed", "2 Sofa Bed Beds").
// Strip any existing trailing "Bed"/"Beds" first so the count suffix is
// applied exactly once, regardless of whether the canonical term already
// has it baked in.
function stripTrailingBedWord(name) { return name.replace(/\s*beds?$/i, ""); }
...
function pass2BeddingSingle(tokens) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    claim(tokens, start, end, "R_BED");
    const bedTypeName = stripTrailingBedWord(t.resolved?.canonical || cap(t.text));
...
// ------------------------------------------------------------
// ENGINE EXTENSION — Building-anchor pairing. A CLASS or VIEW_CORE word
// (an "ambiguous default" word with no fixed home of its own — see the
// long note above buildLookup) sitting directly next to a BUILDING-category
// word is read as a modifier on THAT building, not as a Room class or an
// unpaired view term: "Executive Wing", "Ocean Building", "Club Section",
// "Garden Area" — the modifier attaches to whichever noun it's touching,
// same principle as pass0Access's CLASS-adjective-attachment rule, just
// generalized to a second anchor category (BUILDING) and a second
// attachable word-class (VIEW_CORE, not just CLASS). Order-agnostic: an
// anchor word works on either side ("Tower Suite" / "Suite Tower" already
// worked before this pass even exists, since a lone BUILDING word routes
// straight to the Building bucket regardless of position — this pass only
// matters when there's a modifier word to pull in alongside it).
//
// Deliberately narrow like pass0Access's ACCESS_MODIFIER: exactly one
// modifier word paired with one anchor word. Chained modifiers ("Ocean
// Executive Wing") aren't handled yet — no real supplier example of that
// shape has come up.
// ------------------------------------------------------------
function isBuildingAnchor(t) { return t.resolved?.category === "BUILDING"; }
// ENGINE EXTENSION — words that can attach to a BUILDING anchor as its
// modifier. CLASS/VIEW_CORE were the first two (see the long note above),
// and real supplier data ("East Wing", "West Wing") added a third:
// directional words, their own dedicated category since "east"/"west"
// aren't a Room class or a view term in any other context.
function isBuildingModifierWord(t) { return t.resolved?.category === "CLASS" || t.resolved?.category === "VIEW_CORE" || t.resolved?.category === "BUILDING_MODIFIER"; }

function fmtBuilding(modifierToken, anchorToken) {
  const modVal = modifierToken.resolved?.canonical || cap(modifierToken.text);
  const anchorVal = anchorToken.resolved?.canonical || cap(anchorToken.text);
  return `${modVal} ${anchorVal}`;
}

function pass2BuildingSingle(tokens) {
  for (const { start, end, window } of patternWindows(tokens, 2)) {
    if (window.length !== 2) continue;
    const [a, b] = window;
    if (isBuildingAnchor(a) && isBuildingModifierWord(b)) { claim(tokens, start, end, "R_BUILDING"); return fmtBuilding(b, a); }
    if (isBuildingModifierWord(a) && isBuildingAnchor(b)) { claim(tokens, start, end, "R_BUILDING"); return fmtBuilding(a, b); }
  }
  return null;
}

function pass2BuildingAll(tokens) {
  const results = [];
  let match;
  while ((match = pass2BuildingSingle(tokens)) !== null) results.push(match);
  return results;
}
...
function pass2AlternativeSingle(tokens) {
  for (const { start, end, window } of patternWindows(tokens, 3)) {
    ...
    // ENGINE EXTENSION — a VIEW_MODIFIER sitting directly before the
    // alternation ("Partial City or Sea View") attaches as a prefix on the
    // whole phrase instead of being left stranded. Only handles the
    // modifier-before-the-first-term shape; a modifier attached to the
    // SECOND term only ("Sea or Partial City View") isn't covered here —
    // that needs the right side of the pair to itself be a resolved
    // modifier+core combination, not a bare token, which is a different
    // check. Known scope limit, not attempted yet.
    let claimStart = start;
    let modPrefix = "";
    if (catA === "VIEW_CORE" && start > 0 && tokenAvailable(tokens[start - 1]) && (isViewMod(tokens[start - 1]) || isViewModPositional(tokens[start - 1]))) {
      const modTok = tokens[start - 1];
      modPrefix = `${modTok.resolved?.canonical || cap(modTok.text)} `;
      claimStart = start - 1;
    }

    // ENGINE EXTENSION — when the alternative is between two VIEW_CORE
    // words, also claim a trailing "view" anchor if one directly follows
    // ("sea or pool view"), since the formatted text below already appends
    // " View" itself — without this the anchor word was left dangling and
    // got flagged as unresolved even though it's already represented in
    // the output.
    let claimEnd = end;
    if (catA === "VIEW_CORE" && end < tokens.length && tokenAvailable(tokens[end]) && VIEW_ANCHOR.has(tokens[end].text)) {
      claimEnd = end + 1;
    }
    claim(tokens, claimStart, claimEnd, "R_ALTERNATIVE");
    ...
    if (canonA === canonB) {
      if (catA === "BEDDING_TYPE") return { category: catA, text: `2 ${stripTrailingBedWord(canonA)} Beds`, outputBucket: a.resolved?.outputBucket, accessibilityTypes };
      if (catA === "VIEW_CORE") return { category: catA, text: `${modPrefix}${canonA} View`, outputBucket: a.resolved?.outputBucket, accessibilityTypes };
      return { category: catA, text: canonA, outputBucket: a.resolved?.outputBucket, accessibilityTypes };
    }
    if (catA === "VIEW_CORE") return { category: catA, text: `${modPrefix}${canonA} or ${canonB} View`, outputBucket: a.resolved?.outputBucket, accessibilityTypes };
  ...
  for (const t of tokens) {
    ...
    if (t.resolved) {
      ...
      if (category === "CLASS") buckets.class.push(canonical);
        ...
        // ENGINE EXTENSION — only a word that's genuinely registered as
        // VIEW_CORE (checked via Set membership, same as isViewCore()
        // elsewhere — NOT via `category`, since the dictionary Map's
        // last-write-wins can leave a dual-registered word's `category`
        // pointing at VIEW_MODIFIER even when VIEW_CORE is also a valid
        // reading for it — see Panorama/Panoramic) gets the CLASS
        // fallback: "Panorama Room", "Ocean Suite", "Garden Room". A word
        // that's ONLY ever a connector/modifier (Partial, Facing, Side,
        // Front, Overlooking, Direct) is never a legitimate Room class on
        // its own, so it stays flagged for review instead of being
        // silently promoted. Fixes a real bug: an orphaned "Partial" (e.g.
        // from "partial city or sea view", where the Alternative pass
        // consumes only "city or sea") was wrongly turning into "Partial
        // Room".
        if (canonical && DYNAMIC_VIEW_CORE.has(t.text)) {
          buckets.class.push(canonical);
        } else if (canonical) {
      ...
      } else if (category === "BUILDING_MODIFIER") {
        // ENGINE EXTENSION — a directional word (East/West/...) that never
        // found an adjacent BUILDING anchor to pair with. Unlike the View
        // words above, this doesn't get a CLASS fallback — "east" isn't a
        // Room class in any context — so it's flagged for review instead
        // of guessed at.
        if (canonical) {
          buckets.undefined.push(canonical);
          telemetry?.review({
            kind: "UNPAIRED_SEMANTIC",
            code: "UNPAIRED_BUILDING_MODIFIER",
            token: t.text,
            phase: "pass3",
            explanation: `The dictionary recognized \u2018${canonical}\u2019 as a directional building modifier, but no adjacent building word consumed it.`,
          });
        }
      }
      continue;
...
export async function normalizeCore(rawName, dict, options = {}) {
  ...
  // ENGINE EXTENSION — Building-anchor pairing runs before the alternative/
  // bedding/view passes so a CLASS or VIEW_CORE word touching a BUILDING
  // word (e.g. "Garden" + "Area") is claimed as one Building phrase before
  // anything else has a chance to treat it as an unpaired view term.
  const buildingList = pass2BuildingAll(tokens);
  diag.emit("pass2BuildingAll", "stage.completed", tokens, "Building-anchor pairing pass completed.", { output: buildingList });
  for (let i = 0; i < buildingList.length; i++) telemetry.ruleHit("ENGINE:R_BUILDING");
  ...
  if (buildingList.length) buckets.building.push(...buildingList);
...
export const DICTIONARY_CATEGORIES = [
  ...
  "BUILDING", "BUILDING_MODIFIER", "AMENITY", "PRIVILEGE", "CONDITIONAL", "NOISE", "ACCESS_MODIFIER",
];
