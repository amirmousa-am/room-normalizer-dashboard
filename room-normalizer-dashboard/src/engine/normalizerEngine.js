  ...
  for (const t of tokens) {
    ...
    if (t.resolved) {
      ...
      if (category === "CLASS") buckets.class.push(canonical);
        ...
        // ENGINE EXTENSION — a View-vocabulary word that neither the View-
        // phrase pass nor the Building-pairing pass consumed (no "view"
        // anchor, no modifier partner, no building anchor nearby) defaults
        // to a CLASS reading instead of being flagged UNPAIRED_VIEW: bare
        // "Panorama" -> CLASS "Panorama" (Room bucket, golden case #13),
        // "Ocean Suite" -> CLASS "Ocean" + TYPE "Suite", "Garden Room" the
        // same way. Replaces the old per-word approach of registering the
        // same synonym under both CLASS and VIEW_CORE (which caused the
        // panorama/city/inner bug — see dictionary cleanup migration) with
        // one general rule that covers every current and future View word.
        if (canonical) buckets.class.push(canonical);
