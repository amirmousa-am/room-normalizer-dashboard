...
import { buildLookup, DICTIONARY_CATEGORIES, DICTIONARY_ACTIONS } from "../engine/normalizerEngine";
import { runRegressionSuite } from "../engine/regression";

const EMPTY_FORM = { id: null, synonyms: "", canonical_term: "", category: "AMENITY", action: "REPLACE", priority: 100 };
...
export default function DictionaryManager() {
  ...
  // Deep link from Review Queue: /dictionary?newSynonym=token — opens the
  // "new term" form pre-filled with the flagged token.
  useEffect(() => {
    const newSynonym = searchParams.get("newSynonym");
    if (newSynonym) {
      setForm({ ...EMPTY_FORM, synonyms: newSynonym });
    }
  }, [searchParams]);
