...
const FIELDS = ["class", "occupancy", "type", "bedroom", "bedding", "view", "building", "accessibility", "access", "other", "undefined"];
...
function suggestCause(mismatchedFields, evidence, actualText) {
  ...
  for (const field of mismatchedFields) {
    ...
    if (field.field === "other") {
      suggestions.push({ field: field.field, cause: "CONDITIONAL_LOGIC", note: "Expected a conditional/amenity phrase to be recognized — check whether the exact wording is in the dictionary and, if it's a CONDITIONAL row, whether it has the right output_bucket or trigger_bucket set." });
      continue;
    }
