...
export default function Playground() {
  ...
  async function sendToReviewQueue() {
    ...
    setStatus("Sending to review queue…");
    const { error: rpcError } = await supabase.rpc("collect_normalizer_batch", {
      p_run: { source: "playground", engine_version: ENGINE_VERSION, processed_count: 0 },
      p_review_events: [
        {
          token: selected.text,
          kind: "MANUAL_REVIEW",
          diagnostic_code: "PLAYGROUND_FLAGGED",
          explanation: `Manually flagged from the Playground: token resolved but the output for "${result.raw_name}" looked wrong.`,
          occurrence_count: 1,
          sample_raw_names: [result.raw_name],
        },
      ],
      p_rule_hits: [],
      p_error_events: [],
    });
    setStatus(rpcError ? `Failed: ${rpcError.message}` : "Sent to review queue.");
  }

  async function flagWholeCaseForInvestigation() {
    if (!result) return;
    setStatus("Flagging case for investigation…");
    const token = result.raw_name.toLowerCase().trim();
    const { error: rpcError } = await supabase.rpc("collect_normalizer_batch", {
      p_run: { source: "playground", engine_version: ENGINE_VERSION, processed_count: 0 },
      p_review_events: [
        {
          token,
          kind: "MANUAL_REVIEW",
          diagnostic_code: "PLAYGROUND_CASE_FLAG",
          explanation: `Whole-case flag from the Playground — output for "${result.raw_name}" needs investigation, not necessarily a single token: "${result.canonical_string}"`,
          occurrence_count: 1,
          sample_raw_names: [result.raw_name],
        },
      ],
    ...
    setStatus(rpcError ? `Failed: ${rpcError.message}` : "Case flagged for investigation in Review Queue.");
  }
      ...
      {result && (
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={sendToReviewQueue} disabled={!selected} className="px-3 py-1.5 bg-amber-700 hover:bg-amber-600 disabled:bg-base-700 rounded-md text-xs font-semibold">
              Send selected token to review queue
            </button>
            <button onClick={flagWholeCaseForInvestigation} className="px-3 py-1.5 bg-red-800 hover:bg-red-700 rounded-md text-xs font-semibold">
              Flag whole case for investigation
            </button>
