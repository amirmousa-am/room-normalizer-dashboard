...
function AISuggestionPanel({ row, onApply, busy }) {
  const [state, setState] = useState({ loading: false, error: null, data: null });

  async function ask() {
    setState({ loading: true, error: null, data: null });
    const { data, error } = await supabase.functions.invoke("ai-normalize", {
      body: { token: row.token, sample_raw_name: row.sample_raw_names?.[0] || null },
    });
    if (error) {
      // supabase-js only gives a generic "non-2xx" message by default — the
      // real reason is in the response body on error.context.
      let detail = error.message;
      try {
        const body = await error.context?.json();
        if (body?.error) detail = body.error;
      } catch { /* body wasn't JSON or already consumed — fall back to error.message */ }
      setState({ loading: false, error: detail, data: null });
    }
    else if (!data?.success) setState({ loading: false, error: data?.error || "Unknown error", data: null });
    else setState({ loading: false, error: null, data: data.suggestion });
  }

  const s = state.data;
  return (
    <div className="bg-base-950 border border-blue-900 rounded-md p-3 mt-2 space-y-2 text-xs">
      {!s && (
        <button onClick={ask} disabled={state.loading} className="px-2.5 py-1 bg-blue-600 hover:bg-blue-700 disabled:bg-base-700 rounded text-xs font-semibold">
          {state.loading ? "Asking AI…" : "Ask AI"}
        </button>
      )}
      {state.error && <div className="text-red-400">{state.error}</div>}
      {s && (
        <div className="space-y-1.5">
          <div className="flex justify-between"><span className="text-base-400">Category</span><span className="font-semibold">{s.category}</span></div>
          {s.output_bucket && <div className="flex justify-between"><span className="text-base-400">Output bucket</span><span className="font-semibold">{s.output_bucket}</span></div>}
          {s.trigger_bucket && <div className="flex justify-between"><span className="text-base-400">Trigger bucket</span><span className="font-semibold text-amber-300">{s.trigger_bucket}</span></div>}
          <div className="flex justify-between"><span className="text-base-400">Canonical term</span><span className="font-semibold">{s.canonical_term}</span></div>
          <div className="flex justify-between">
            <span className="text-base-400">Confidence</span>
            <span className={s.confidence >= 0.75 ? "text-green-400" : s.confidence >= 0.5 ? "text-yellow-300" : "text-red-400"}>{Math.round(s.confidence * 100)}%</span>
          </div>
          <div className="text-base-400 italic">"{s.reasoning}"</div>
          <div className="flex justify-end gap-2 pt-1">
            <button onClick={ask} disabled={state.loading} className="px-2 py-1 text-base-400 hover:text-base-50">Ask again</button>
            <Link
              to={`/dictionary?newSynonym=${encodeURIComponent(row.token)}&category=${encodeURIComponent(s.category)}&canonical=${encodeURIComponent(s.canonical_term)}`}
              className="px-2.5 py-1 bg-green-700 hover:bg-green-600 rounded text-xs font-semibold"
            >
    ...
    <div className="border border-base-800 rounded-md px-3 py-2.5">
      <div className="flex justify-between items-center gap-3">
        ...
        <div className="flex gap-1.5 whitespace-nowrap">
          ...
          <button onClick={() => setAsking((a) => !a)} className="text-xs px-2 py-1 bg-blue-900 hover:bg-blue-800 rounded text-blue-200">
            Ask AI
          </button>
      ...
      {asking && <AISuggestionPanel row={row} busy={busy} />}
    </div>
  );
}
